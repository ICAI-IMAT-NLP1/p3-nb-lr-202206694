...
class LogisticRegression:
    ...
    def fit(
        ...
        for epoch in range(epochs):
            linear_combination = features @ self._weights
            predictions = self.sigmoid(linear_combination)
            loss = self.binary_cross_entropy_loss(predictions, labels)
            gradient = features.T @ (predictions - labels) / labels.size(0)
            self._weights -= learning_rate * gradient
        #self._weights = torch.tensor([0.2222, 0.0288, 0.4485, 0.2301], dtype=torch.float32)
        return
    ...
    def initialize_parameters(self, dim: int, random_state: int) -> torch.Tensor:
        ...
        params: torch.Tensor = torch.randn(dim + 1, dtype=torch.float32) * 0.01
        return params
